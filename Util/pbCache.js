    ...
    try {
        ...
        for (i=0; i < rows1.length; i++) {
            ...
            if (row[8] !== '') {
                ...
                if (!gravs1[row[7]][row[5]]) gravs1[row[7]][row[5]] = {};
        ...
        for (i=0; i < rows2.length; i++) {
            ...
            if (row[8] !== '') {
                ...
                if (!gravs2[row[7]][row[5]]) gravs2[row[7]][row[5]] = {};
        ...
        for (i=0; i < rows3.length; i++) {
            ...
            if (row[8] !== '') {
                ...
                if (!gravs3[row[7]][row[5]]) gravs3[row[7]][row[5]] = {};
        ...
        console.log('An error occurred in pbCache[setpbCache]: ' + err);
    }
}

function updatePbCache(data) {
    if (!cache[data.sesason][data.mode][data.map]) cache[data.season][data.mode][data.map] = {};
    if (!cache[data.sesason][data.mode][data.map][data.user]) cache[data.season][data.mode][data.map][data.user] = {};
    cache[data.season][data.mode][data.map][data.user].user = data.user;
    cache[data.season][data.mode][data.map][data.user].time = data.time;
    cache[data.season][data.mode][data.map][data.user].link = data.link;
    const date = new Date();
    const day = `${date.getDate}/${date.getMonth}/${date.getFullYear}`;
    cache[data.season][data.mode][data.map][data.user].date = day;
}
